...
Singleton {
  ...
  function deleteLocalWallpaper(path) {
    ...
    fileDeleter.command = ["rm", path];
    fileDeleter.running = true;
  }

  function deleteLocalWallpapers(paths) {
      if (!paths || paths.length === 0) return;
      Logger.i("Wallpaper", "Deleting wallpapers batch:", paths.length);
      var cmd = ["rm", "-f"];
      for (var i = 0; i < paths.length; i++) {
          cmd.push(paths[i]);
      }
      fileDeleter.command = cmd;
      fileDeleter.running = true;
  }
  ...
  Process {
    ...
    onExited: exitCode => {
      if (exitCode === 0) {
        Logger.i("Wallpaper", "Deleted wallpaper(s) successfully");
        root.refreshWallpapersList();
      } else {
        Logger.e("Wallpaper", "Failed to delete wallpaper(s). Exit code:", exitCode, "Stderr:", stderr.text);
      }
    }
  }
